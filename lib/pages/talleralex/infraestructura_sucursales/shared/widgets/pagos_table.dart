...
class _PagosTableState extends State<PagosTable> {
          ...
          children: [
            ...
            Expanded(
              child: PlutoGrid(
                ...
                rows: widget.provider.pagosRows,
                onLoaded: (PlutoGridOnLoadedEvent event) {
                  stateManager = event.stateManager;
                  _configureGrid();
                },
                onRowDoubleTap: (event) => _mostrarDetallePago(event.row),
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  '${widget.provider.pagosFiltrados.length} pagos encontrados',
    ...
    if (isSmallScreen) {
      ...
      columns.addAll([
        ...
        PlutoColumn(
          ...
          enableSorting: true,
          enableColumnDrag: false,
          renderer: (rendererContext) => _buildMetodoRenderer(
            rendererContext.cell.value?.toString() ?? '',
            theme,
          ),
        ),
        ...
        PlutoColumn(
          ...
          enableSorting: true,
          enableColumnDrag: false,
          renderer: (rendererContext) => _buildFacturaRenderer(
            rendererContext.cell.value?.toString() ?? '',
            theme,
          ),
        ),
  ...
  Widget _buildMetodoRenderer(String metodo, AppTheme theme) {
    IconData icon;
    Color color;
  ...
  Widget _buildFacturaRenderer(String factura, AppTheme theme) {
    final tieneFactura = factura.isNotEmpty && factura != 'null';
    ...
    return Container(
      ...
      child: Row(
        children: [
          Icon(
            tieneFactura ? Icons.receipt : Icons.receipt_outlined,
            size: 14,
            color: tieneFactura ? Colors.green : Colors.grey,
          ),
          const SizedBox(width: 6),
          Expanded(
            child: Text(
              tieneFactura ? factura : 'Sin factura',
              style: TextStyle(
                fontFamily: 'Poppins',
                fontSize: 13,
                color: tieneFactura ? theme.primaryText : Colors.grey,
              ),
              overflow: TextOverflow.ellipsis,
            ),
          ),
              ...
              itemBuilder: (context) => [
                const PopupMenuItem(
                  ...
                  child: ListTile(
                    ...
                    title: Text('Imprimir', style: TextStyle(fontSize: 12)),
                    dense: true,
                  ),
                ),
                const PopupMenuItem(
  ...
  void _mostrarDialogoNuevoPago() {
    // TODO: Implementar diálogo de nuevo pago
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
          content: Text('Funcionalidad de nuevo pago en desarrollo')),
    );
  }
  ...
  void _mostrarDetallePago(PlutoRow row) {
    // TODO: Implementar vista de detalles
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
          content: Text('Ver detalles del pago: ${row.cells['orden']?.value}')),
    );
  }

  void _editarPago(PlutoRow row) {
    // TODO: Implementar edición
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text('Editar pago: ${row.cells['orden']?.value}')),
    );
  }

  void _generarFactura(PlutoRow row) {
    // TODO: Implementar generación de factura
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
          content: Text('Generar factura para: ${row.cells['orden']?.value}')),
    );
  }
  ...
  void _ejecutarAccion(String accion, PlutoRow row) {
    switch (accion) {
      case 'imprimir':
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text('Imprimir pago: ${row.cells['orden']?.value}')),
        );
        break;
      case 'duplicar':
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text('Duplicar pago: ${row.cells['orden']?.value}')),
        );
        break;
      case 'eliminar':
  ...
  void _confirmarEliminacion(PlutoRow row) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
            '¿Estás seguro de que deseas eliminar el pago ${row.cells['orden']?.value}?'),
        actions: [
          TextButton(
